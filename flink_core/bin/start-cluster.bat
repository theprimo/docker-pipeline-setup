...
SET zookeeperUrl=localhost:2181
SET namespace=ns1
SET apiPath=/opt/flink/conf/openapi.json
SET dsConfigPath=/opt/flink/conf/datasources.json
...
::start java -server -javaagent:/opt/flink/lib/javaflow.instrument-continuations.jar %JVM_ARGS% %log_setting_tm% -DnodeId=node1 -cp "%FLINK_CLASSPATH%"; org.apache.flink.runtime.taskexecutor.TaskManagerRunner --configDir "%FLINK_CONF_DIR%" > "%out_tm%" 2>&1

start java -agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=5005  -server -javaagent:/opt/flink/lib/javaflow.instrument-continuations.jar %JVM_ARGS% %log_setting_tm% -DnodeId=node1 -cp "%FLINK_CLASSPATH%"; org.apache.flink.runtime.taskexecutor.TaskManagerRunner --configDir "%FLINK_CONF_DIR%" > "%out_tm%" 2>&1
